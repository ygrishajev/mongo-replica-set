...
ARG DB_URL=https://fastdl.mongodb.org/linux/mongodb-linux-x86_64-ubuntu2004-5.0.8.tgz

RUN apt-get update && \
    apt-get upgrade -y && \
    apt-get install -y curl && \
    curl -OL ${DB_URL} && \
    tar -zxvf mongodb-linux-x86_64-ubuntu2004-5.0.8.tgz && \
    mv ./mongodb-linux-x86_64-ubuntu2004-5.0.8/bin/* /usr/local/bin/ && \
    rm -rf ./mongodb-linux-x86_64-ubuntu2004-5.0.8 && rm ./mongodb-linux-x86_64-ubuntu2004-5.0.8.tgz
